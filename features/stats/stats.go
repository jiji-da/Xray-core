...
func GetOrRegisterRateLimiter(m Manager, name string, rateLimit int) (*rate.Limiter, error) {
	limiter := m.GetRateLimiter(name)
	if limiter != nil {
		return limiter, nil
	}

	return m.RegisterRateLimiter(name, rateLimit)
}
